def generate_explanation(accuracy, coefficients):
    ...
    explanation = f"El modelo alcanzó una precisión del {round(accuracy*100,1)}%. "
    if top_positive:
        explanation += f"Variables que más aumentan la probabilidad de la clase positiva: {', '.join(top_positive)}. "
    if top_negative:
    ...
    if accuracy > 0.99:
        ...
        explanation += " El rendimiento es bajo, se recomienda revisar características."
    else:
